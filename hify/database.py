    ...
    def delete_orphan_tracks(self, valid_paths: set):
        with self._connect() as conn:
            try:
                ...
                conn.commit()
                if deleted > 0:
                    logger.info(f"Cleaned up {deleted} orphaned tracks from DB.")
    ...
    def save_track_metadata(self, track_id: str, data: dict):
        with self._connect() as conn:
            try:
                ...
                artist_name = data.get('artist') or (data.get('artists')[0] if data.get('artists') else '')
                ...
                if album_id and album_name:
                    conn.execute(
                        ...
                        (album_id, album_name, data.get('cover_url'), data.get('year'))
                    )
